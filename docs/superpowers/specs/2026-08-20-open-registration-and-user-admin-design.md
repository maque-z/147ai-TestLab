...
`iat` 按 RFC 7519 为**整秒**，而 `password_changed_at` 是微秒精度 datetime。这一秒的模糊地带在两个方向上各有一个失败模式，必须同时处理。

**方向一（可用性）**：若管理员重置密码后用户在同一秒内登录，新 token 的 `iat` 因向下取整而小于 `password_changed_at`，刚拿到的 token 立即失效 —— 表现为"登录成功后马上被踢出"。

**方向二（安全性，实施期间实测发现）**：反过来，重置前**同一秒内**签发的旧 token 会满足 `iat == floor(password_changed_at)`，因而通过 `<` 比较。且由于该比较是无状态的，这枚 token **在其完整 7 天有效期内一直有效**，并非只逃过一次。

方向二不是理论风险。重置密码的典型场景恰恰是"密码已泄漏，要把对方踢出去"，而对方**持有旧密码**：他只要每秒登录一次，重置发生的那一秒必然有一枚属于他的 token，该 token 完整存活。重置就此失去它唯一的作用。

**处理**：比较时把 `password_changed_at` 向下取整到秒（解决方向一），并在 `reset_password` 写入时**向上进位到下一个整秒**（解决方向二）：

```python
user.password_changed_at = utcnow().replace(microsecond=0) + timedelta(seconds=1)
```

进位使重置所在的那一整秒**整体不可达** —— 该秒内签发的任何 token 都被拒，第一个可接受的 `iat` 是下一秒。

**代价权衡**：恰好落在重置那一秒内的合法登录也会被拒一次，重试即可通过 —— 有界且自愈。实测中这一情形甚至难以触发：bcrypt 自身的开销（cost=12，每次约 250ms）通常已把后续登录推到下一秒。用这个代价换掉一个不会自愈的安全漏洞是合算的。

不能改用 `<=` 比较：那会让方向一重新出现，且没有解决方向二 —— 两者都发生在同一秒内，`<=` 无法区分。
